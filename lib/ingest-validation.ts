...
export function validateProduct(p: RawProduct): ValidationResult {
  ...
  if (p.price !== undefined && (!Number.isFinite(p.price) || p.price <= 0 || p.price > 5000)) warnings.push(`price out of range: ${p.price}`);
...
export function parseDimensions(d: RawProduct["dimensions"]): Record<string, unknown> | null {
  ...
  if (m) return { length: Number(m[1]), width: Number(m[2]), height: Number(m[3]), unit: m[4]?.toLowerCase() === "centimeters" ? "cm" : (m[4] || "in"), raw: d };
  return { raw: d };
}
